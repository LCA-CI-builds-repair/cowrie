...
import json
from configparser import NoOptionError  # Unused import
...
class Output(cowrie.core.output.Output):
    ...
    def sendLogs(self, logentry):
        ...
        except oci.exceptions.ServiceError as ex:  # Use logging instead of print
            self.log(
                f"Oracle Cloud plugin Error: {ex.message}\n" +
                f"Oracle Cloud plugin Status Code: {ex.status}\n"
            )
        except Exception as ex:  # Use logging instead of print
            self.log(f"Oracle Cloud plugin Error: {ex}")
            raise
